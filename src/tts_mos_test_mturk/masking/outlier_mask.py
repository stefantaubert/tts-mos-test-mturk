...
def mask_outliers_alg(ratings: np.ndarray, max_std_dev_diff: float) -> np.ndarray:
  ...
  for alg_i in range(n_alg):
    ...
    result[alg_i, :] = masked
    dlogger.info(f"Algorithm {alg_i}:")
    dlogger.info(f"- min: {np.nanmin(mu_norm)}")
    dlogger.info(f"- mean: {np.nanmean(mu_norm)}")
    dlogger.info(f"- median: {np.nanmedian(mu_norm)}")
    dlogger.info(f"- max: {np.nanmax(mu_norm)}")
    dlogger.info(f"- masked: {np.sum(masked)}")
  return result
